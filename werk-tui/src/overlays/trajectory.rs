...
impl WerkApp {
    pub(crate) fn render_trajectory_overlay(&self, area: Rect, frame: &mut Frame<'_>) {
        let line_count = self.trajectory_lines.len() as u16;

        let bg_style = Style::new().fg(CLR_LIGHT_GRAY).bg(CLR_BG_DARK);
        let content = Paragraph::new(Text::from_lines(self.trajectory_lines.clone())).style(bg_style);
