...
fn test_show_json_has_temporal_signals() {
    ...
    let tension = store
        .create_tension_full("test goal", "test reality", None, Some(h))
        .unwrap();
    ...
    assert!(
        json["frontier"]["closure_progress"].is_object(),
        "should have closure_progress"
    );

    // Should NOT have dynamics field
    assert!(
        json.get("dynamics").is_none(),
        "dynamics field should not exist"
    );
}
