...
/// Running `werk add` from subdirectory uses parent's .werk/werk.db
#[test]
fn test_discovery_add_from_subdirectory() {
    ...
    let store = werk_core::Store::init_unlocked(parent_dir.path()).unwrap();
...
fn test_discovery_reality_from_subdirectory() {
    ...
    let updated = werk_core::Store::init_unlocked(parent_dir.path())
        .unwrap()
...
fn test_discovery_resolve_from_subdirectory() {
    ...
    let resolved = werk_core::Store::init_unlocked(parent_dir.path())
        .unwrap()
        .get_tension(&tension.id)
        .unwrap()
        .unwrap();
    assert_eq!(resolved.status, werk_core::TensionStatus::Resolved);
}
...
fn test_discovery_release_from_subdirectory() {
    ...
    let released = werk_core::Store::init_unlocked(parent_dir.path())
        .unwrap()
        .get_tension(&tension.id)
        .unwrap()
        .unwrap();
    assert_eq!(released.status, werk_core::TensionStatus::Released);
}
...
fn test_discovery_rm_from_subdirectory() {
    ...
    let deleted = werk_core::Store::init_unlocked(parent_dir.path())
        .unwrap()
        .get_tension(&tension.id)
        .unwrap();
...
fn test_discovery_move_from_subdirectory() {
    ...
    let moved = werk_core::Store::init_unlocked(parent_dir.path())
        .unwrap()
...
fn test_discovery_note_from_subdirectory() {
    ...
    let mutations = werk_core::Store::init_unlocked(parent_dir.path())
        .unwrap()
...
fn test_global_fallback_add() {
    ...
    let _store = werk_core::Store::init_unlocked(home_dir.path()).unwrap();
    ...
    let store = werk_core::Store::init_unlocked(home_dir.path()).unwrap();
    let tensions = store.list_tensions().unwrap();
    assert_eq!(tensions.len(), 1);
...
fn test_global_fallback_reality() {
    ...
    // Verify in global store
    let updated = werk_core::Store::init_unlocked(home_dir.path())
        .unwrap()
        .get_tension(&tension.id)
        .unwrap()
        .unwrap();
...
fn test_workspace_consistency_across_commands() {
    ...
    let store = werk_core::Store::init_unlocked(project_root.path()).unwrap();
    let tensions = store.list_tensions().unwrap();
...
fn test_local_precedence_over_global() {
    ...
    let global_werk = home_dir.path().join(".werk");
    std::fs::create_dir_all(&global_werk).unwrap();
    let global_store = werk_core::Store::init_unlocked(home_dir.path()).unwrap();
    global_store
    ...
    let local_store = werk_core::Store::init_unlocked(project_dir.path()).unwrap();
    ...
    let global_tensions = werk_core::Store::init_unlocked(home_dir.path())
        .unwrap()
        .list_tensions()
        .unwrap();
...
fn test_error_recovery_resolve_twice() {
    ...
    let store_after_first = werk_core::Store::init_unlocked(dir.path()).unwrap();
    ...
    let store_after_second = werk_core::Store::init_unlocked(dir.path()).unwrap();
...
fn test_error_recovery_release_no_reason() {
    ...
    // Get initial state
    let before = werk_core::Store::init_unlocked(dir.path())
        .unwrap()
        .get_tension(&tension.id)
        .unwrap()
        .unwrap();
    ...
    // State unchanged
    let after = werk_core::Store::init_unlocked(dir.path())
        .unwrap()
        .get_tension(&tension.id)
        .unwrap()
        .unwrap();
...
fn test_error_recovery_rm_not_found() {
    ...
    // Get initial state
    let before_count = werk_core::Store::init_unlocked(dir.path())
        .unwrap()
        .list_tensions()
        .unwrap()
        .len();
    ...
    let after_count = werk_core::Store::init_unlocked(dir.path())
        .unwrap()
        .list_tensions()
        .unwrap()
        .len();
    ...
    let still_exists = werk_core::Store::init_unlocked(dir.path())
        .unwrap()
        .get_tension(&tension.id)
        .unwrap();
...
fn test_error_recovery_move_cycle() {
    ...
    // Get state before
    let before_a = werk_core::Store::init_unlocked(dir.path())
        .unwrap()
        .get_tension(&a.id)
        .unwrap()
        .unwrap();
    let before_c = werk_core::Store::init_unlocked(dir.path())
        .unwrap()
        .get_tension(&c.id)
        .unwrap()
        .unwrap();
    ...
    // State unchanged
    let after_a = werk_core::Store::init_unlocked(dir.path())
        .unwrap()
        .get_tension(&a.id)
        .unwrap()
        .unwrap();
    let after_c = werk_core::Store::init_unlocked(dir.path())
        .unwrap()
        .get_tension(&c.id)
        .unwrap()
        .unwrap();
...
fn test_prefix_consistency_across_commands() {
    ...
    let final_tension = werk_core::Store::init_unlocked(dir.path())
        .unwrap()
        .get_tension(&tension.id)
        .unwrap()
        .unwrap();
    ...
    // Check mutations
    let mutations = werk_core::Store::init_unlocked(dir.path())
        .unwrap()
        .get_mutations(&tension.id)
        .unwrap();
    assert!(
...
fn test_prefix_move_and_release() {
    ...
    // Verify move
    let moved = werk_core::Store::init_unlocked(dir.path())
        .unwrap()
    ...
    // Verify release
    let released = werk_core::Store::init_unlocked(dir.path())
        .unwrap()
...
fn test_prefix_resolve_and_rm() {
    ...
    // Verify resolve
    let resolved = werk_core::Store::init_unlocked(dir.path())
        .unwrap()
        .get_tension(&t1.id)
        .unwrap()
        .unwrap();
    ...
    // Verify rm
    let deleted = werk_core::Store::init_unlocked(dir.path())
        .unwrap()
        .get_tension(&t2.id)
        .unwrap();
    assert!(deleted.is_none());
...
fn test_deep_nesting_discovery() {
    ...
    let store = werk_core::Store::init_unlocked(root.path()).unwrap();
    let tensions = store.list_tensions().unwrap();
    assert_eq!(tensions.len(), 1);
