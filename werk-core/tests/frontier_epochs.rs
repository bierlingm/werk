...
fn test_resolved_children_appear_after_epoch_boundary() {
    ...
    let c1 = store
        .create_tension_with_parent("step 1", "todo", Some(parent.id.clone()))
        .unwrap();
    let c2 = store
        .create_tension_with_parent("step 2", "todo", Some(parent.id.clone()))
        .unwrap();
    let c3 = store
        .create_tension_with_parent("step 3", "todo", Some(parent.id.clone()))
        .unwrap();

    // Resolve c1 before epoch
    store
        .update_status(&c1.id, TensionStatus::Resolved)
        .unwrap();

    // Create epoch
    std::thread::sleep(std::time::Duration::from_millis(10));
    store
        .create_epoch(&parent.id, "project", "started", None, None)
        .unwrap();
    ...
    let c1_status_ts = store
        .get_last_mutation_timestamps(&[c1.id.as_str()], &["status"])
        .unwrap();
    let c2_status_ts = store
        .get_last_mutation_timestamps(&[c2.id.as_str()], &["status"])
        .unwrap();
...
fn test_no_epochs_means_all_resolved_visible() {
    ...
    let parent = store.create_tension("project", "started").unwrap();
    let c1 = store
        .create_tension_with_parent("step 1", "done", Some(parent.id.clone()))
        .unwrap();
    let c2 = store
        .create_tension_with_parent("step 2", "done", Some(parent.id.clone()))
        .unwrap();
    ...
    let statuses = store
        .get_last_mutation_timestamps(&[c1.id.as_str(), c2.id.as_str()], &["status"])
        .unwrap();
...
fn test_epoch_close_partitions_accumulated() {
    ...
    let c1 = store
        .create_tension_with_parent("task a", "todo", Some(parent.id.clone()))
        .unwrap();
    let c2 = store
        .create_tension_with_parent("task b", "todo", Some(parent.id.clone()))
        .unwrap();
    let c3 = store
        .create_tension_with_parent("task c", "todo", Some(parent.id.clone()))
        .unwrap();
    ...
    store
        .create_epoch(&parent.id, "project v2", "v1", None, None)
        .unwrap();
...
fn test_count_children_by_parent_returns_correct_counts() {
    ...
    store
        .create_tension_with_parent("c1a", "r", Some(p1.id.clone()))
        .unwrap();
    store
        .create_tension_with_parent("c1b", "r", Some(p1.id.clone()))
        .unwrap();
    store
        .create_tension_with_parent("c1c", "r", Some(p1.id.clone()))
        .unwrap();
    store
        .create_tension_with_parent("c2a", "r", Some(p2.id.clone()))
        .unwrap();

    let counts = store
        .count_children_by_parent(&[p1.id.as_str(), p2.id.as_str()])
        .unwrap();
...
fn test_batch_mutation_timestamps_returns_per_tension() {
    ...
    let timestamps = store
        .get_last_mutation_timestamps(&[t1.id.as_str(), t2.id.as_str()], &["actual", "created"])
        .unwrap();
